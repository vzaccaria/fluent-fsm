# Makefile generated automatically on July 31st 2013, 11:49:25 am
...
SERVER_DIR=./lib/.
CLIENT_DIR=./lib/static
CLIENT_DIR_IMG=./lib/static/img
CLIENT_DIR_FONTS=./lib/static/font
...
# 
SERVER_SOURCES= \
	$(strip $(patsubst %, ./build/%, $(patsubst %.ls, %.js, $(notdir ./src/fsmexpress.ls)))) \
	$(strip $(patsubst %, ./build/%, $(patsubst %.ls, %.js, $(notdir ./test/test.ls))))
...
server: 
	 ...
	 @watchman -w ./test 'touch ./.client-changed' & echo "$$!" >> "./.watches.pid" 
	 ...
	 @watchman -r 3s -w ./.client-changed 'make test' & echo "$$!" >> "./.watches.pid"
...
.PHONY: test
test: 
	 mocha ./lib/./$(strip $(patsubst %.ls, %.js, $(notdir ./test/test.ls))) --reporter spec
